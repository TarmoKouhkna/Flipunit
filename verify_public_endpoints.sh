...
if [ $FAILED -eq 0 ]; then
    ...
    echo "      ssh ubuntu@217.146.78.140 'cd /opt/flipunit && cat flipunit/middleware.py'"
    echo ""
    echo "   2. Check if middleware is registered:"
    echo "      ssh ubuntu@217.146.78.140 'cd /opt/flipunit && grep middleware flipunit/settings.py'"
    echo ""
    echo "   3. Restart web container:"
    echo "      ssh ubuntu@217.146.78.140 'cd /opt/flipunit && docker-compose restart web'"
    echo ""
    echo "   4. Check container logs:"
    echo "      ssh ubuntu@217.146.78.140 'cd /opt/flipunit && docker-compose logs web | tail -50'"
    exit 1
fi
